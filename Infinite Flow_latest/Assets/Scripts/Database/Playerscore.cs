...
public class Playerscore : MonoBehaviour
{
    ...
    public GameObject signin_feed_back;
    public GameObject signup_feed_back;
    ...
    void Start()
    {
        if (password_signupText != null && password_loginText != null)
        {
            if (password_signupText != null && password_loginText != null)
            {
    ...
    {
        signup_feed_back.SetActive(true);
        signup_feed_back.GetComponent<Text>().text = "Signing Up...";

        string temp_nickname = Sign_up();
        /*new*/
        //RestClient.Get("https://infiflow-f2fab.firebaseio.com/" + ".json").Then(response =>
        RestClient.Get("https://infiflow-f2fab.firebaseio.com/" + "users" + ".json").Then(response =>       // SIGN UP SUCCESFULL GOTO MAIN MEN
        {
            string boss = response.Text;
            var data = (JObject)JsonConvert.DeserializeObject(boss);
            if (data[playerid_signup] != null)
            {
                signup_feed_back.GetComponent<Text>().text = "Player ID already in use!";
                Is_submitted.text = "Player ID already in use!";
            }
            else
            {
                PostToDatabase(temp_nickname);
            }
        });
    ...
    private void PostToDatabase(string nickname)
    {
    ...
    public void OnGet()
    {
        signin_feed_back.SetActive(true);
        signin_feed_back.GetComponent<Text>().text = "Signing In...";
    ...
    public void GetterfromDatabase()
    {
        ...
        RestClient.Get("https://infiflow-f2fab.firebaseio.com/" + "users/" + ".json").Then(response1 =>
        {
            ...
            {
                print("dashfduashfusahfusahfuashfuahfushf");
        ...
        RestClient.Get<playerinfo>("https://infiflow-f2fab.firebaseio.com/" + "users/" + playerid_login + ".json").Then(response =>
        {
            ...
            if (password_login == new_player_get.password)
            {
                ...
                */
                // GOTO main menu
                print("hamzah");
                NickName.nickName = new_player_get.nickname;
            ...
            {
                signin_feed_back.GetComponent<Text>().text = "Invalid Username or Password!";
    ...
    public void minhal()
    {
        signin_feed_back.SetActive(false);
    }
    public void hamzah()
    {
        signup_feed_back.SetActive(false);
    }
